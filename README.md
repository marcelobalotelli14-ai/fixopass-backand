...
- **Webhook de pagamento**: `POST /webhooks/payment` — corpo `{ companyId, ... }`, autenticado por segredo compartilhado no header `X-WEBHOOK-SECRET` (env var `PAYMENT_WEBHOOK_SECRET`). Em caso de sucesso, marca `status: 'ACTIVE'` e soma 30 dias a `nextDueDate` (a partir do `nextDueDate` atual, se ainda estiver no futuro, senão a partir de agora). **Atenção**: isso NÃO é verificação de assinatura oficial do Mercado Pago/Asaas (HMAC/token do gateway) — é só a proteção mínima (um segredo compartilhado) até uma integração real entrar. Ver comentário detalhado em `src/routes/webhooks.ts` antes de processar pagamento de verdade.
